...
int main(void) {
    ...
    //sem_unlink("ex16sem1Altere");
    //sem_unlink("ex16sem2Altere");
    //sem_unlink("ex16sem3Altere");
    //shm_unlink("/pl4ex16Alterb");
    ...
    if ((acessoEx = sem_open("ex16sem1Altere", O_CREAT, 0644, 1)) == SEM_FAILED) {
        exit(1);
    }
    if ((sentido1PodePassar = sem_open("ex16sem2Altere", O_CREAT, 0644, 0)) == SEM_FAILED) {
        exit(1);
    }
    if ((sentido2PodePassar = sem_open("ex16sem3Altere", O_CREAT, 0644, 0)) == SEM_FAILED) {
        exit(1);
    }
    int meu_sentido = 1;
    sem_wait(acessoEx);
    shared_data->nrCarrosSentido1++;
    if (shared_data->ocupado == 1 && shared_data->sentidoAtivo != meu_sentido){
        sem_post(acessoEx);
        printf("Estou à espera de poder passar a ponte!\n");
        sem_wait(sentido1PodePassar);
    } else {
    ...
    if (shared_data->nrCarrosSentido1 == 0){
        if (shared_data->nrCarrosSentido2 != 0){
            shared_data->sentidoAtivo = 2;
        } else {
            shared_data->ocupado = 0;
            shared_data->sentidoAtivo = 0;
        }
        int i;
        for(i = 0; i < shared_data->nrCarrosSentido2; i++){
            sem_post(sentido2PodePassar);
        }
    }
    sem_post(acessoEx);
